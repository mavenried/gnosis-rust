...
Only the modules needed to render reflowable and fixed-layout EPUBs are
vendored here — traced from `view.js`'s own import graph, not the whole
upstream repo (which also includes comic/FB2/MOBI/PDF loaders, OPDS, a
dictionary popup, search, and TTS, none of which Gnosis uses):
